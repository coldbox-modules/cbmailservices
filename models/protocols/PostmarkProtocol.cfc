...
<cfcomponent
	...
	output ="false"
	hint   ="A mail protocol that sends via http://postmarkapp.com/"
>
	<!--- init --->
	<cffunction
		name      ="init"
		access    ="public"
		returntype="PostmarkProtocol"
		hint      ="Constructor"
		output    ="false"
	>
		<cfargument
	...
	>
		<cfargument
			name    ="mail"
			required="true"
			type    ="cbmailservices.models.Mail"
			hint    ="The mail payload to send."
		/>
		...
		<cfset local.jsonPacketWithoutPrefix = trim(
			mid(
				local.jsonPacket,
				find( "{", local.jsonPacket ),
				len( local.jsonPacket )
			)
		)/>
		...
		--->
		<cfset local.rtnStruct = reformatPostMarkResponse(
			deserializeJSON( local.cfhttp.FileContent.toString() )
		)/>
	...
	<cffunction
		...
		access    ="private"
		returntype="struct"
		hint      ="I encode named headers so that PostMark likes it"
	>
		<cfargument
			name    ="MailParam"
			required="true"
			type    ="struct"
			hint    ="I'm the file path for the attachment."
		/>
	...
	<cffunction
		...
		access    ="private"
		returntype="struct"
		hint      ="I encode an attachment so that PostMark likes it."
	>
		<cfargument
			name    ="MailParam"
			required="true"
	...
	<cffunction
		...
		returntype="string"
		output    ="false"
		hint      ="I calculate the MIME type for a given file."
	>
		<cfargument
			name    ="filePath"
			type    ="string"
			required="true"
			hint    ="I'm the path to the file to be tested."
		/>
	...
	<cffunction
		name      ="reformatPostMarkResponse"
		access    ="private"
		returntype="struct"
		hint      ="I format the PostMarkApp return result into one which conforms to the coldbox mailer service."
	>
		<cfargument
			name    ="PostMarkReturnStruct"
			required="true"
			type    ="struct"
			hint    ="I'm the returned structure from postmark"
		/>
