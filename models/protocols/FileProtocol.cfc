...
component extends="cbmailservices.models.AbstractProtocol" accessors="true" {
    ...
    struct function send( required cbmailservices.models.Mail payload ) {
        ...
        try {
            // write it out
            fileWrite( filePath, getMailContent( arguments.payload ), "UTF-8" );
            // send success
    ...
    private function getMailContent( required mail ) {
		...
		savecontent variable="local.thisMail" {
			writeOutput(
				'<div hidden data-cbmailservices-log data-from="#encodeForHTMLAttribute( arguments.mail.getFrom() )#" data-to="#encodeForHTMLAttribute( arguments.mail.getTo() )#" data-subject="#encodeForHTMLAttribute( arguments.mail.getSubject() )#" data-sent="#encodeForHTMLAttribute( dateTimeFormat( now(), "iso" ) )#"></div>'
			);
			writeOutput( "
