<cfcomponent extends="coldbox.system.testing.BaseTestCase" output="false">
	...
	<cffunction name="setup" access="public" output="false" returntype="void">
		<cfscript>
			...
			props = { filePath="/test/specs/protocols/tmp" };

			// Create a mock instance of the protocol.
			protocol =  getMockBox().createMock(className="mailservices.model.protocols.FileProtocol").init(props);
		</cfscript>
	</cffunction>
	...
	<cffunction name="testSend" access="public" output="false" returntype="void">
		...
		<cfdirectory action="list" directory="#expandPath('/test/specs/protocols/tmp')#" filter="*.html" listinfo="name" name="qTesting" >

		<cfset debug(qTesting)>
		<cfset AssertTrue( qTesting.recordcount )>

		<cfloop query="qTesting">
			<cfset fileDelete( expandPath('/test/specs/protocols/tmp/#qTesting.name#') )>
		</cfloop>
