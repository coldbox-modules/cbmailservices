<cfcomponent extends="coldbox.system.testing.BaseTestCase" output="false">
	...
	<cffunction name="setup" access="public" output="false" returntype="void">
		<cfscript>
			...
			protocol =  getMockBox().createMock(className="mailservices.models.protocols.postmarkProtocol").init(props);
		</cfscript>
	</cffunction>
	...
	<cffunction name="testSend" access="public" output="false" returntype="void">
		<cfscript>
			...
			payload = getMockBox().createMock(className="mailservices.models.Mail").init().config(from="info@coldbox.org",to="automation@coldbox.org",type="html");
			...
			// 2:Mail with params
			payload = getMockBox().createMock(className="mailservices.models.Mail").init().config(from="info@coldbox.org",to="automation@coldbox.org",subject="Mail With Params - Hello Luis");
			...
			payload = getMockBox().createMock(className="mailservices.models.Mail").init().config(from="info@coldbox.org",to="automation@coldbox.org",subject="Mail MultiPart No Params - Hello Luis");
			...
			payload = getMockBox().createMock(className="mailservices.models.Mail").init().config(from="info@coldbox.org",to="automation@coldbox.org",subject="Mail MultiPart With Params - Hello Luis");
			payload.addMailPart(type="text",body="You are reading this message as plain text, because your mail reader does not handle it.");
			payload.addMailPart(type="html",body="This is the body of the message.");
			payload.addMailParam(name="Disposition-Notification-To",value="info@coldbox.org");
			rtn = protocol.send(payload);
